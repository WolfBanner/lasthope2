...
async def async_execute(
    ...
    for f in (process.stdout, process.stderr):
        ...
        for char in iter(lambda: f.readline(), b""):
            ...
            decoded_char = re.sub(r'\s*\(\d+\s*bytes\s*\)', '', decoded_char)
            stream.append(decoded_char)
            if on_read:
                await on_read(stream, process, *args)
            if time.time() - start > 1800:
                raise MegaError(0, "Account limits exceeded, wait a few hours before start any Mega download again")
        streams.append(stream)
    return (*to_string(*streams), process.wait())
